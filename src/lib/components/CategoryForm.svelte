<script lang="ts">
    ...
    import {type CategorySchema, categorySchema} from '$lib/schemas/expense';
	...
	interface Props {
        formData: SuperValidated<Infer<CategorySchema>>;
		isEdit?: boolean;
		categoryGroups?: Array<{id: string, name: string, icon?: string}>;
	}

	let { formData, isEdit = false, categoryGroups = [] }: Props = $props();
...
<form method="POST" use:enhance class="space-y-6">
	...
	<div>
		...
		<Select
			...
			class="w-full"
		>
			<option value="">No Group (Standalone Category)</option>
		...
		<p class="mt-1 text-xs text-gray-500">
			Optional: Assign this category to a group for better organization
		</p>
	</div>
