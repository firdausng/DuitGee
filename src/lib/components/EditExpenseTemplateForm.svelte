<script lang="ts">
    ...
    import type {Category, ExpenseTemplate, UpdateExpenseTemplate} from "$lib/schemas/expense";
    ...
    // Form state - pre-filled with template data
    let formData: UpdateExpenseTemplate = $state({
        ...
        defaultUserId: template.defaultUserId !== undefined ? template.defaultUserId : '__creator__',
    ...
    let defaultUser = $state(template.defaultUserId !== undefined ? template.defaultUserId : '__creator__');
    ...
    // Payment type
    let paymentType = $state(template.paymentType || "");
    ...
    // Payment provider
    let searchPaymentProviderValue = $state(template.paymentProvider || "");
    ...
    // Icon
    let iconValue = $state(template.icon || "📝");
    function getIcon() {
        return iconValue;
    }
...
<form onsubmit={handleSubmit} class="space-y-4">
    ...
    <div>
        ...
        <Combobox.Root
            ...
            onOpenChangeComplete={(o) => {if (!o) categorySearchValue = formData.categoryName ?? "";}}
        >
            <div class="relative">
    ...
    {#if paymentType?.length > 0}
        <div>
            ...
            <Combobox.Root
                ...
                onOpenChangeComplete={(o) => {if (!o) searchPaymentProviderValue = formData.paymentProvider ?? "";}}
            >
                <div class="relative">
